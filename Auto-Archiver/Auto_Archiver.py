...
#        print(f"Parsed URL: {parsed}, Path Parts: {path_parts}")  # Debugging line
    ...
    def main(args):
        ...
        #print(args)
        # Set console title
        chaname = AutoArchiver.get_channel_name(args[0]) if args else "Anon's channel"
        title_str = f"Archiving {chaname}'s streams as they happen..."
        if os.name == 'nt':
            os.system(f'title {title_str}')
        else:
            sys.stdout.write(f"\x1b]2;{title_str}\x07")
        AutoArchiver.yt_dlp_handler()
        AutoArchiver.lurker(args)
    ...
    def lurker(channel):
        first_run = 1
        chaname = AutoArchiver.get_channel_name(channel[0])
        yt_settings = AutoArchiver.loadSettings()

        # Change working directory to the save location from yt_settings[0]
        save_location = yt_settings[0][1]
        workingFolder = AutoArchiver.find_or_create_channel_folder(save_location,chaname)
        print(workingFolder)
        if os.path.isdir(save_location):
            # Find or create the channel folder
            channel_folder = AutoArchiver.find_or_create_channel_folder(save_location, chaname)
        ...
        while 1:
            ...
            AutoArchiver.clear_screen()
            
            if first_run or current_time % check_this_time == 0:
                if not first_run:
                    check_this_time = check_again_interval[random.randint(0, len(check_again_interval) - 1)]
                first_run = 0
                gatVids = AutoArchiver.getVideoURL(channel)
                is_multidimensional = (
                ...
                if not len(gatVids) ==0:
                    ...
                    title = AutoArchiver.getVideoTitle(currVid[0])
                    print(fullURL)
                    if  AutoArchiver.checkTime(int(currVid[1])):
    ...
    def getVideoURL(args):
        url = args[0]+"/streams"
  #      print(url)
        channel_name = AutoArchiver.get_channel_name(args[0])
#        print(channel_name)
        try:
